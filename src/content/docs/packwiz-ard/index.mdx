...
Load and update your modpack with ease. PackWiz-ard is a mod that supports the PackWiz ecosystem by default, while also
supporting Packweave as an alternative backend. With PackWiz-ard, you can keep your modpack up to date with the latest
versions of mods and easily manage your pack source.
...
update notifications, and the ability to easily update the client-side mods to match the active modpack source.
...
That screen shows the current pack link validation result, whether a client update is available, and whether the client needs to be restarted.
