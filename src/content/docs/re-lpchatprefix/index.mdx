...
The config is located at `<game_directory>/config/relpchatprefix/config.json` and by default will look like the following:

```json title="<game_directory>/config/relpchatprefix/config.json"
{
  "mainConfig": {
    "enablePrefix": true,
    "enableSuffix": true,
