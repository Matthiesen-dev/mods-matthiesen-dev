---
title: Cobble Poke Bank - Configuration
description: Configuration for Cobble Poke Bank.
prev: 
  link: /cobble-poke-bank
  label: "Overview"
next: 
  link: /cobble-poke-bank/commands
  label: "Commands"
sidebar:
  label: Configuration
  order: 2
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 6
---

All of the configuration files for Cobble Poke Bank are located in the `<game_directory>/config/cobble_poke_bank/` directory.

**Configuration files:**
- `database.toml` - Contains the database configuration settings for Cobble Poke Bank. This file allows you to configure the database type (SQLite or MySQL), connection details, and other related settings.
- `permissions.toml` - Contains the permission settings for Cobble Poke Bank. This file allows you to define the permissions required for various actions within the mod, such as accessing the bank, depositing or withdrawing Pokémon, and managing other players' Pokémon.
- `server.toml` - Contains the server configuration settings for Cobble Poke Bank. This file allows you to configure server-specific settings, such as enabling or disabling certain features, setting limits on the number of Pokémon that can be stored, and other server-related options.

## Database Configuration

```toml title="database.toml"
#Database Configuration
[database]
	useMySQL = false

	#MySQL Configuration
	[database.mysql]
		host = "localhost"
		port = 3306
		database = "cobble_poke_bank"
		username = "root"
		password = "password"
		timeout = 5000
```

### useMySQL

The `useMySQL` setting determines whether the mod will use a MySQL database or a local SQLite database for storage. Set it to `true` to use MySQL, or `false` to use SQLite.

- **Default:** `false` (uses SQLite)

### MySQL Configuration

The MySQL configuration section contains the settings required to connect to a MySQL database. These settings are only relevant if `useMySQL` is set to `true`.

#### host

The `host` setting specifies the hostname or IP address of the MySQL server.

- **Default:** `localhost`

#### port

The `port` setting specifies the port number of the MySQL server.

- **Default:** `3306`
- **Range:** `1 ~ 65535`

#### database

The `database` setting specifies the name of the MySQL database to use.

- **Default:** `cobble_poke_bank`

#### username

The `username` setting specifies the username to use when connecting to the MySQL server.

- **Default:** `root`

#### password

The `password` setting specifies the password to use when connecting to the MySQL server.

- **Default:** `password`

#### timeout

The `timeout` setting specifies the timeout in milliseconds for connecting to the MySQL server.

- **Default:** `5000`
- **Range:** `> 1`

## Permissions Configuration

```toml title="permissions.toml"
#Permissions Configuration
[permissions]

	#Command Permissions
	[permissions.command]
		pokebank = "NONE"
		pokebank_status = "ALL_COMMANDS"
		pokebank_reload = "ALL_COMMANDS"
```

### Command Permissions

Configure the permission levels required to use the commands provided by Cobble Poke Bank.

**Available Permission Levels:**
- `NONE` - No permission required.
- `SPAWN_PROTECTION_BYPASS` - Requires the player to have spawn protection bypass permission.
- `CHEAT_COMMANDS_AND_COMMAND_BLOCKS` - Requires the player to have cheat commands and command blocks permission.
- `MULTIPLAYER_MANAGEMENT` - Requires the player to have multiplayer management permission.
- `ALL_COMMANDS` - Requires the player to have permission to use all commands.

#### pokebank

The `pokebank` permission level determines the required permission to use the `/pokebank` command.

- **Default:** `NONE`

#### pokebank_status

The `pokebank_status` permission level determines the required permission to use the `/pokebank status` command.

- **Default:** `ALL_COMMANDS`

#### pokebank_reload

The `pokebank_reload` permission level determines the required permission to use the `/pokebank reload` command.

- **Default:** `ALL_COMMANDS`

## Server Configuration

```toml title="server.toml"
#Server config
[server]

	#Bank Configuration Options
	[server.bank]
		maxSlots = -1
		noFainted = false
		noHeldItems = false
		noLegendaries = false
		noMythicals = false
		noUltraBeasts = false
		pokemonBlacklist = []

	#Held Item Restrictions
	[server.heldItemRestrictions]
		officialTaggedOnly = false
		autoStrip = false
		blacklist = []

	#GUI Configuration
	[server.gui]
		frameItemId = "minecraft:gray_stained_glass_pane"
		pcItemId = "cobblemon:pc"
		bankItemId = "minecraft:ender_chest"
		infoItemId = "minecraft:paper"
		navPrevItemId = "minecraft:arrow"
		navNextItemId = "minecraft:arrow"
		backItemId = "minecraft:oak_sign"
		confirmItemId = "minecraft:lime_dye"
		cancelItemId = "minecraft:red_dye"
		invalidItemId = "minecraft:barrier"

		#GUI Text Configuration
		[server.gui.text]
			buttonWithdraw = "Move Pokemon to PC?"
			buttonDeposit = "Move Pokemon to bank?"
			buttonConfirmTransfer = "Confirm Transfer"
			mainMenuInfo = "Move Pokemon between PC and Bank"
			mainMenuTitle = "{player}'s Poke Bank"
			userBankScreenTitle = "{player}'s Bank"
			userPCScreenTitle = "{player}'s PC"
			buttonOpenPC = "Open PC"
			buttonOpenBank = "Open Bank"
			buttonBack = "Back"
			buttonPreviousPage = "Previous"
			buttonNextPage = "Next"
			buttonConfirm = "Confirm"
			buttonCancel = "Cancel"
			buttonInvalidEntry = "Invalid Pokemon Data"
			pokemonMovesListLabel = "Moves:"
			pokemonEmptyMoveSlot = "Empty"
			pokemonNoNickname = "No nickname"
			pokemonNoHeldItem = "No held item"
			pokemonOTUnknown = "Unknown"
			pokemonIVs = "IVs:"
			pokemonEVs = "EVs:"
			pokemonLevel = "Level:"
			pokemonNickname = "Nickname:"
			pokemonHeldItem = "Held Item:"
			pokemonOT = "OT:"
			pokemonNature = "Nature:"
			pokemonAbility = "Ability:"
			pokemonForm = "Form:"

			[server.gui.text.pokemonStats]
				hp = "HP"
				attack = "Atk"
				defense = "Def"
				specialAttack = "SpAtk"
				specialDefense = "SpDef"
				speed = "Spd"
				evasion = "Evasion"
				accuracy = "Accuracy"

	#Messages Configuration
	[server.messages]
		messagesTextParser = "VANILLA"
		messagePrefix = "§f[§6Cobble Poke Bank§f] §r"

		#Messages - Location Labels
		[server.messages.locationLabels]
			deposit = "Bank"
			withdraw = "Server"

		#Messages - Command Messages
		[server.messages.commandMessages]
			databaseUnavailable = "§cDatabase is not available. Please try again later."
			playerNotFound = "§cFailed to find executing player."
			noBlacklistedItems = "§eNo held items are blacklisted."
			inBattle = "§cYou cannot access the bank while in battle."
			configsReloaded = "§aConfigs reloaded successfully."
			noBlacklistedPokemon = "§eNo Pokemon are blacklisted."
			invalidConfigType = "§cInvalid config type. Valid types are: %s"
			invalidConfigName = "§cInvalid config name. Valid names are: %s"
			invalidIntegerValue = "§cInvalid value for %s. Please provide a valid integer."
			invalidBooleanValue = "§cInvalid value for %s. Please provide a valid boolean (true/false)."
			configUpdated = "§aUpdated %s successfully."

		#Messages - Database Messages
		[server.messages.databaseMessages]
			loadingData = "§eLoading bank data..."
			failedToLoadData = "§cFailed to load bank data. Please try again later."
			invalidPokemonData = "§cInvalid Pokemon entry. Check server logs."

		#Messages - Deposit Messages
		[server.messages.depositMessages]
			pokemonMissing = "§cPokemon is no longer in your PC"
			failedToSave = "§cFailed to save Pokemon to bank. Please try again later."
			bankFull = "§cYour bank is full. Please remove a Pokemon before depositing another."
			pcRemovalFailed = "§cFailed to remove Pokemon from PC. Please try again later."
			pokemonDeposited = "§aPokemon has been moved to your Bank."

		#Messages - Withdraw Messages
		[server.messages.withdrawMessages]
			pokemonMissing = "§cPokemon is no longer in your Bank."
			failedToReadData = "§cFailed to read Pokemon data from bank. Please try again later."
			failedToRemoveFromBank = "§cFailed to remove Pokemon from bank. Please try again later."
			pcFull = "§cYour PC is full. Please remove a Pokemon before withdrawing another."
			pokemonDeposited = "§aPokemon has been moved to your PC."

		#Messages - Validation Messages
		[server.messages.validationMessages]
			noHeldItems = "§cThis Pokemon is holding an item, which is not allowed in the %s."
			officialHeldItemsOnly = "§cThis Pokemon is holding an item that is not officially tagged, which is not allowed in the %s."
			blacklistedHeldItem = "§cThis Pokemon is holding a blacklisted item, which is not allowed in the %s."
			noLegendaries = "§cThis Pokemon is a legendary, which is not allowed in the %s."
			noMythicals = "§cThis Pokemon is a mythical, which is not allowed in the %s."
			noUltraBeasts = "§cThis Pokemon is an Ultra Beast, which is not allowed in the %s."
			noFainted = "§cThis Pokemon is fainted, which is not allowed in the %s."
			autoStrippedHeldItem = "§eYour Pokemon's held item has been removed as it is not allowed in the %s."

		#Messages - Status Display
		[server.messages.statusDisplay]
			title = "Cobble Poke Bank Status"
			sectionDatabase = "Database"
			sectionBankConfiguration = "Bank Configuration"
			pokemonBlacklistTitle = "Cobble Poke Bank Pokemon Blacklist"
			sectionBlacklistedPokemon = "Blacklisted Pokemon"
			heldItemBlacklistTitle = "Cobble Poke Bank Held Item Blacklist"
			sectionBlacklistedHeldItems = "Blacklisted Held Items"
			labelDatabaseType = "Database Type"
			labelDatabaseStatus = "Database Status"
			labelBankMaxSlots = "Bank Max Slots"
			labelNoFainted = "No Fainted"
			labelNoHeldItems = "No Held Items"
			labelNoLegendaries = "No Legendaries"
			labelNoMythicals = "No Mythicals"
			labelNoUltraBeasts = "No Ultra Beasts"
			labelPokemonBlacklistEntries = "Pokemon Blacklist Entries"
			labelOfficialHeldItemsOnly = "Official Held Items Only"
			labelHeldItemBlacklistEntries = "Held Item Blacklist Entries"
			valueMySQL = "MySQL"
			valueSQLite = "SQLite"
			valueConnected = "§aConnected"
			valueOffline = "§cOffline"
			valueUnlimited = "Unlimited"
			valueEnabled = "§aEnabled"
			valueDisabled = "§cDisabled"

```

### Bank Configuration Options

Configure the bank settings for Cobble Poke Bank.

#### maxSlots

The `maxSlots` setting determines the maximum number of slots a player can have in their bank. A value of `-1` means unlimited storage.

- **Default:** `-1` (unlimited storage)

#### noFainted

The `noFainted` setting determines whether players are allowed to store fainted Pokémon in the bank. Set it to `true` to disallow fainted Pokémon, or `false` to allow them.

- **Default:** `false` (fainted Pokémon are allowed)

#### noHeldItems

The `noHeldItems` setting determines whether players are allowed to store Pokémon with held items in the bank. Set it to `true` to disallow Pokémon with held items, or `false` to allow them.

- **Default:** `false` (Pokémon with held items are allowed)

#### noLegendaries

The `noLegendaries` setting determines whether players are allowed to store Legendary Pokémon in the bank. Set it to `true` to disallow Legendary Pokémon, or `false` to allow them.

- **Default:** `false` (Legendary Pokémon are allowed)

#### noMythicals

The `noMythicals` setting determines whether players are allowed to store Mythical Pokémon in the bank. Set it to `true` to disallow Mythical Pokémon, or `false` to allow them.

- **Default:** `false` (Mythical Pokémon are allowed)

#### noUltraBeasts

The `noUltraBeasts` setting determines whether players are allowed to store Ultra Beast Pokémon in the bank. Set it to `true` to disallow Ultra Beast Pokémon, or `false` to allow them.

- **Default:** `false` (Ultra Beast Pokémon are allowed)

#### pokemonBlacklist

The `pokemonBlacklist` setting allows you to specify a list of Pokémon that are not allowed to be stored in the bank. Players will not be able to store these Pokémon in the bank.

> **Note:** The Pokémon should be specified using their internal names without the `cobblemon:` prefix (e.g., `pikachu`, `charizard`, etc.).

- **Default:** `[]` (no Pokémon are blacklisted)

### Held Item Restrictions

Configure the held item restrictions for Cobble Poke Bank.

#### officialTaggedOnly

The `officialTaggedOnly` setting determines whether players are allowed to store Pokémon with held items that are not officially tagged. Set it to `true` to disallow Pokémon with untagged held items, or `false` to allow them.

- **Default:** `false` (Pokémon with untagged held items are allowed)

#### autoStrip

The `autoStrip` setting determines whether players will have their Pokémon's held items automatically stripped if they are not allowed in the bank. Set it to `true` to automatically strip held items, or `false` to disallow storing Pokémon with unallowed held items.

- **Default:** `false` (Pokémon with unallowed held items are not automatically stripped)

#### blacklist

The `blacklist` setting allows you to specify a list of held items that are not allowed to be stored in the bank. Players will not be able to store Pokémon with these held items in the bank.

- **Default:** `[]` (no held items are blacklisted)

### GUI Configuration

Configure the GUI settings for Cobble Poke Bank.

#### frameItemId

The `frameItemId` setting specifies the item ID to use for the frame of the bank GUI.

- **Default:** `minecraft:gray_stained_glass_pane`

#### pcItemId

The `pcItemId` setting specifies the item ID to use for the PC button in the bank GUI.

- **Default:** `cobblemon:pc`

#### bankItemId

The `bankItemId` setting specifies the item ID to use for the Bank button in the bank GUI.

- **Default:** `minecraft:ender_chest`

#### infoItemId

The `infoItemId` setting specifies the item ID to use for the Info button in the bank GUI.

- **Default:** `minecraft:paper`

#### navPrevItemId

The `navPrevItemId` setting specifies the item ID to use for the Previous button in the bank GUI.

- **Default:** `minecraft:arrow`

#### navNextItemId

The `navNextItemId` setting specifies the item ID to use for the Next button in the bank GUI.

- **Default:** `minecraft:arrow`

#### backItemId

The `backItemId` setting specifies the item ID to use for the Back button in the bank GUI.

- **Default:** `minecraft:oak_sign`

#### confirmItemId

The `confirmItemId` setting specifies the item ID to use for the Confirm button in the bank GUI.

- **Default:** `minecraft:lime_dye`

#### cancelItemId

The `cancelItemId` setting specifies the item ID to use for the Cancel button in the bank GUI.

- **Default:** `minecraft:red_dye`

#### invalidItemId

The `invalidItemId` setting specifies the item ID to use for the Invalid button in the bank GUI.

- **Default:** `minecraft:barrier`

#### GUI Text Configuration

Configuration for the text displayed in the Cobble Poke Bank GUI. You can customize the text for various buttons, labels, and messages shown to players.

##### buttonWithdraw

The `buttonWithdraw` setting specifies the text to display on the Withdraw button in the bank GUI.

- **Default:** `Move Pokemon to PC?`

##### buttonDeposit

The `buttonDeposit` setting specifies the text to display on the Deposit button in the bank GUI.

- **Default:** `Move Pokemon to bank?`

##### buttonConfirmTransfer

The `buttonConfirmTransfer` setting specifies the text to display on the Confirm Transfer button in the bank GUI.

- **Default:** `Confirm Transfer`

##### mainMenuInfo

The `mainMenuInfo` setting specifies the text to display on the Info button in the main menu GUI.

- **Default:** `Move Pokemon between PC and Bank`

##### mainMenuTitle

The `mainMenuTitle` setting specifies the text to display as the title of the main menu GUI. You can use `{player}` as a placeholder for the player's name.

- **Default:** `{player}'s Poke Bank`

##### userBankScreenTitle

The `userBankScreenTitle` setting specifies the text to display as the title of the user bank screen GUI. You can use `{player}` as a placeholder for the player's name.

- **Default:** `{player}'s Bank`

##### userPCScreenTitle

The `userPCScreenTitle` setting specifies the text to display as the title of the user PC screen GUI. You can use `{player}` as a placeholder for the player's name.

- **Default:** `{player}'s PC`

##### buttonOpenPC

The `buttonOpenPC` setting specifies the text to display on the Open PC button in the main menu GUI.

- **Default:** `Open PC`

##### buttonOpenBank

The `buttonOpenBank` setting specifies the text to display on the Open Bank button in the main menu GUI.

- **Default:** `Open Bank`

##### buttonBack

The `buttonBack` setting specifies the text to display on the Back button in the main menu GUI.

- **Default:** `Back`

##### buttonPreviousPage

The `buttonPreviousPage` setting specifies the text to display on the Previous Page button in the main menu GUI.

- **Default:** `Previous`

##### buttonNextPage

The `buttonNextPage` setting specifies the text to display on the Next Page button in the main menu GUI.

- **Default:** `Next`

##### buttonConfirm

The `buttonConfirm` setting specifies the text to display on the Confirm button in the main menu GUI.

- **Default:** `Confirm`

##### buttonCancel

The `buttonCancel` setting specifies the text to display on the Cancel button in the main menu GUI.

- **Default:** `Cancel`

##### buttonInvalidEntry

The `buttonInvalidEntry` setting specifies the text to display on the Invalid Entry button in the main menu GUI.

- **Default:** `Invalid Pokemon Data`

##### pokemonMovesListLabel

The `pokemonMovesListLabel` setting specifies the text to display as the label for the Pokémon moves list in the main menu GUI.

- **Default:** `Moves:`

##### pokemonEmptyMoveSlot

The `pokemonEmptyMoveSlot` setting specifies the text to display for empty move slots in the Pokémon moves list in the main menu GUI.

- **Default:** `Empty`

##### pokemonNoNickname

The `pokemonNoNickname` setting specifies the text to display when a Pokémon has no nickname in the main menu GUI.

- **Default:** `No nickname`

##### pokemonNoHeldItem

The `pokemonNoHeldItem` setting specifies the text to display when a Pokémon has no held item in the main menu GUI.

- **Default:** `No held item`

##### pokemonOTUnknown

The `pokemonOTUnknown` setting specifies the text to display when a Pokémon's original trainer is unknown in the main menu GUI.

- **Default:** `Unknown`

##### pokemonIVs

The `pokemonIVs` setting specifies the text to display as the label for a Pokémon's IVs in the main menu GUI.

- **Default:** `IVs:`

##### pokemonEVs

The `pokemonEVs` setting specifies the text to display as the label for a Pokémon's EVs in the main menu GUI.

- **Default:** `EVs:`

##### pokemonLevel

The `pokemonLevel` setting specifies the text to display as the label for a Pokémon's level in the main menu GUI.

- **Default:** `Level:`

##### pokemonNickname

The `pokemonNickname` setting specifies the text to display as the label for a Pokémon's nickname in the main menu GUI.

- **Default:** `Nickname:`

##### pokemonHeldItem

The `pokemonHeldItem` setting specifies the text to display as the label for a Pokémon's held item in the main menu GUI.

- **Default:** `Held Item:`

##### pokemonOT

The `pokemonOT` setting specifies the text to display as the label for a Pokémon's original trainer in the main menu GUI.

- **Default:** `OT:`

##### pokemonNature

The `pokemonNature` setting specifies the text to display as the label for a Pokémon's nature in the main menu GUI.

- **Default:** `Nature:`

##### pokemonAbility

The `pokemonAbility` setting specifies the text to display as the label for a Pokémon's ability in the main menu GUI.

- **Default:** `Ability:`

##### pokemonForm

The `pokemonForm` setting specifies the text to display as the label for a Pokémon's form in the main menu GUI.

- **Default:** `Form:`

### Messages Configuration

Configure the messages sent to players by Cobble Poke Bank.

#### messagesTextParser

The `messagesTextParser` setting determines the text parser to use for messages sent to players. You can choose from the following options:
- `VANILLA` - Uses the default Minecraft text formatting.
- `EMBERS` - Uses the Embers Text API for advanced text formatting.
- `ADVENTURE` - Uses the Adventure MiniMessage format for advanced text formatting.

* **Default:** `VANILLA` (uses default Minecraft text formatting)

#### messagePrefix

The `messagePrefix` setting allows you to specify a prefix that will be added to all messages sent to players by Cobble Poke Bank. This can be used to provide context or branding for the messages.

- **Default:** `§f[§6Cobble Poke Bank§f] §r` (displays the mod name in gold and white text)

#### locationLabels

The `locationLabels` setting allows you to specify custom labels for the deposit and withdraw locations in messages sent to players. This can be used to provide context or branding for the messages.

##### deposit

The `deposit` label specifies the label to use for the deposit location in messages sent to players.

- **Default:** `Bank`

##### withdraw

The `withdraw` label specifies the label to use for the withdraw location in messages sent to players.

- **Default:** `Server`

#### commandMessages

The `commandMessages` setting allows you to specify custom messages for various command-related events in Cobble Poke Bank. These messages will be sent to players when certain events occur, such as when the database is unavailable or when a player tries to use a command they don't have permission for.

##### databaseUnavailable

The `databaseUnavailable` message is sent to players when the database is unavailable.

- **Default:** `§cDatabase is not available. Please try again later.`

##### playerNotFound

The `playerNotFound` message is sent to players when the executing player cannot be found.

- **Default:** `§cFailed to find executing player.`

##### noBlacklistedItems

The `noBlacklistedItems` message is sent to players when they try to deposit a Pokémon with a held item that is not blacklisted.

- **Default:** `§eNo held items are blacklisted.`

##### inBattle

The `inBattle` message is sent to players when they try to use the bank while in battle.

- **Default:** `§cYou cannot access the bank while in battle.`

##### configsReloaded

The `configsReloaded` message is sent to players when the configuration files are successfully reloaded.

- **Default:** `§aConfigs reloaded successfully.`

##### noBlacklistedPokemon

The `noBlacklistedPokemon` message is sent to players when they try to deposit a Pokémon that is not blacklisted.

- **Default:** `§eNo Pokemon are blacklisted.`

##### invalidConfigType

The `invalidConfigType` message is sent to players when the configuration file contains an invalid type.

- **Default:** `§cInvalid configuration type. Check server logs.`

##### invalidConfigName

The `invalidConfigName` message is sent to players when the configuration file contains an invalid name.

- **Default:** `§cInvalid configuration name. Check server logs.`

##### invalidIntegerValue

The `invalidIntegerValue` message is sent to players when the configuration file contains an invalid integer value.

- **Default:** `§cInvalid integer value. Check server logs.`

##### invalidBooleanValue

The `invalidBooleanValue` message is sent to players when the configuration file contains an invalid boolean value.

- **Default:** `§cInvalid boolean value. Check server logs.`

##### configUpdated

The `configUpdated` message is sent to players when the configuration file is successfully updated.

- **Default:** `§aConfiguration updated successfully.`

#### databaseMessages

The `databaseMessages` setting allows you to specify custom messages for various database-related events in Cobble Poke Bank. These messages will be sent to players when certain events occur, such as when the database is loading data or when there is an error reading or writing data.

##### loadingData

The `loadingData` message is sent to players when the database is loading data.

- **Default:** `§eLoading bank data...`

##### failedToLoadData

The `failedToLoadData` message is sent to players when the database fails to load data.

- **Default:** `§cFailed to load bank data. Please try again later.`

##### invalidPokemonData

The `invalidPokemonData` message is sent to players when the database has invalid Pokémon data.

- **Default:** `§cInvalid Pokemon entry. Check server logs.`

#### depositMessages

The `depositMessages` setting allows you to specify custom messages for various deposit-related events in Cobble Poke Bank. These messages will be sent to players when certain events occur, such as when they try to deposit a Pokémon that is missing or when the bank is full.

##### pokemonMissing

The `pokemonMissing` message is sent to players when they try to deposit a Pokémon that is missing.

- **Default:** `§cPokemon is no longer in your PC`

##### failedToSave

The `failedToSave` message is sent to players when the database fails to save the deposited Pokémon.

- **Default:** `§cFailed to save Pokemon to bank. Please try again later.`

##### bankFull

The `bankFull` message is sent to players when they try to deposit a Pokémon but the bank is full.

- **Default:** `§cYour bank is full. Please remove a Pokemon before depositing another.`

##### pcRemovalFailed

The `pcRemovalFailed` message is sent to players when the database fails to remove the deposited Pokémon from the PC.

- **Default:** `§cFailed to remove Pokemon from PC. Please try again later.`

##### pokemonDeposited

The `pokemonDeposited` message is sent to players when they successfully deposit a Pokémon.

- **Default:** `§aPokemon has been moved to your Bank.`

#### withdrawMessages

The `withdrawMessages` setting allows you to specify custom messages for various withdraw-related events in Cobble Poke Bank. These messages will be sent to players when certain events occur, such as when they try to withdraw a Pokémon that is missing or when their PC is full.

##### pokemonMissing

The `pokemonMissing` message is sent to players when they try to withdraw a Pokémon that is missing.

- **Default:** `§cPokemon is no longer in your Bank.`

##### failedToReadData

The `failedToReadData` message is sent to players when the database fails to read the withdrawn Pokémon's data.

- **Default:** `§cFailed to read Pokemon data from bank. Please try again later.`

##### failedToRemoveFromBank

The `failedToRemoveFromBank` message is sent to players when the database fails to remove the withdrawn Pokémon from the bank.

- **Default:** `§cFailed to remove Pokemon from bank. Please try again later.`

##### pcFull

The `pcFull` message is sent to players when they try to withdraw a Pokémon but their PC is full.

- **Default:** `§cYour PC is full. Please remove a Pokemon before withdrawing another.`

##### pokemonDeposited

The `pokemonDeposited` message is sent to players when they successfully withdraw a Pokémon.

- **Default:** `§aPokemon has been moved to your PC.`

#### validationMessages

The `validationMessages` setting allows you to specify custom messages for various validation-related events in Cobble Poke Bank. These messages will be sent to players when certain events occur, such as when they try to deposit a Pokémon with a held item that is not allowed or when they try to deposit a Legendary Pokémon when Legendary Pokémon are not allowed.

##### noHeldItems

The `noHeldItems` message is sent to players when they try to deposit a Pokémon with a held item when held items are not allowed.

- **Default:** `§cThis Pokemon is holding an item, which is not allowed in the %s.`

##### officialHeldItemsOnly

The `officialHeldItemsOnly` message is sent to players when they try to deposit a Pokémon with a held item that is not officially tagged when only officially tagged held items are allowed.

- **Default:** `§cThis Pokemon is holding an item that is not officially tagged, which is not allowed in the %s.`

##### blacklistedHeldItem

The `blacklistedHeldItem` message is sent to players when they try to deposit a Pokémon with a held item that is blacklisted.

- **Default:** `§cThis Pokemon is holding a blacklisted item, which is not allowed in the %s.`

##### noLegendaries

The `noLegendaries` message is sent to players when they try to deposit a Legendary Pokémon when Legendary Pokémon are not allowed.

- **Default:** `§cThis Pokemon is a legendary, which is not allowed in the %s.`

##### noMythicals

The `noMythicals` message is sent to players when they try to deposit a Mythical Pokémon when Mythical Pokémon are not allowed.

- **Default:** `§cThis Pokemon is a mythical, which is not allowed in the %s.`

##### noUltraBeasts

The `noUltraBeasts` message is sent to players when they try to deposit an Ultra Beast Pokémon when Ultra Beast Pokémon are not allowed.

- **Default:** `§cThis Pokemon is an Ultra Beast, which is not allowed in the %s.`

##### noFainted

The `noFainted` message is sent to players when they try to deposit a fainted Pokémon when fainted Pokémon are not allowed.

- **Default:** `§cThis Pokemon is fainted, which is not allowed in the %s.`

##### autoStrippedHeldItem

The `autoStrippedHeldItem` message is sent to players when they try to deposit a Pokémon with a held item that is not allowed and the held item is automatically stripped.

- **Default:** `§eYour Pokemon's held item has been removed as it is not allowed in the %s.`

#### Status Display Messages

The `statusDisplay` setting allows you to specify custom messages for the status display of Cobble Poke Bank. These messages will be shown to players when they use the `/pokebank status` command to view the current status of the bank, including database connection, bank configuration, and blacklisted Pokémon and held items.

##### title

The `title` message is shown as the title of the `/pokebank status` table.

- **Default:** `Cobble Poke Bank Status`

##### sectionDatabase

The `sectionDatabase` message is shown as the section header for the database information in the `/pokebank status` table.

- **Default:** `Database`

##### sectionBankConfiguration

The `sectionBankConfiguration` message is shown as the section header for the bank configuration information in the `/pokebank status` table.

- **Default:** `Bank Configuration`

##### pokemonBlacklistTitle

The `pokemonBlacklistTitle` message is shown as the title for the Pokémon blacklist table in the `/pokebank status` command.

- **Default:** `Cobble Poke Bank Pokemon Blacklist`

##### sectionBlacklistedPokemon

The `sectionBlacklistedPokemon` message is shown as the section header for the blacklisted Pokémon in the `/pokebank status` command.

- **Default:** `Blacklisted Pokemon`

##### heldItemBlacklistTitle

The `heldItemBlacklistTitle` message is shown as the title for the held item blacklist table in the `/pokebank status` command.

- **Default:** `Cobble Poke Bank Held Item Blacklist`

##### sectionBlacklistedHeldItems

The `sectionBlacklistedHeldItems` message is shown as the section header for the blacklisted held items in the `/pokebank status` command.

- **Default:** `Blacklisted Held Items`

##### labelDatabaseType

The `labelDatabaseType` message is shown as the label for the database type row in the `/pokebank status` table.

- **Default:** `Database Type`

##### labelDatabaseStatus

The `labelDatabaseStatus` message is shown as the label for the database status row in the `/pokebank status` table.

- **Default:** `Database Status`

##### labelBankMaxSlots

The `labelBankMaxSlots` message is shown as the label for the bank max slots row in the `/pokebank status` table.

- **Default:** `Bank Max Slots`

##### labelNoFainted

The `labelNoFainted` message is shown as the label for the no fainted row in the `/pokebank status` table.

- **Default:** `No Fainted`

##### labelNoHeldItems

The `labelNoHeldItems` message is shown as the label for the no held items row in the `/pokebank status` table.

- **Default:** `No Held Items`

##### labelNoLegendaries

The `labelNoLegendaries` message is shown as the label for the no legendaries row in the `/pokebank status` table.

- **Default:** `No Legendaries`

##### labelNoMythicals

The `labelNoMythicals` message is shown as the label for the no mythicals row in the `/pokebank status` table.

- **Default:** `No Mythicals`

##### labelNoUltraBeasts

The `labelNoUltraBeasts` message is shown as the label for the no ultra beasts row in the `/pokebank status` table.

- **Default:** `No Ultra Beasts`

##### labelPokemonBlacklistEntries

The `labelPokemonBlacklistEntries` message is shown as the label for the Pokémon blacklist entry count row in the `/pokebank status` table.

- **Default:** `Pokemon Blacklist Entries`

##### labelOfficialHeldItemsOnly

The `labelOfficialHeldItemsOnly` message is shown as the label for the official held items only row in the `/pokebank status` table.

- **Default:** `Official Held Items Only`

##### labelHeldItemBlacklistEntries

The `labelHeldItemBlacklistEntries` message is shown as the label for the held item blacklist entry count row in the `/pokebank status` table.

- **Default:** `Held Item Blacklist Entries`

##### valueMySQL

The `valueMySQL` message is shown as the value for the database type row in the `/pokebank status` table when MySQL is in use.

- **Default:** `MySQL`

##### valueSQLite

The `valueSQLite` message is shown as the value for the database type row in the `/pokebank status` table when SQLite is in use.

- **Default:** `SQLite`

##### valueConnected

The `valueConnected` message is shown as the value for the database status row in the `/pokebank status` table when the database is connected.

- **Default:** `§aConnected`

##### valueOffline

The `valueOffline` message is shown as the value for the database status row in the `/pokebank status` table when the database is offline.

- **Default:** `§cOffline`

##### valueUnlimited

The `valueUnlimited` message is shown as the value for the bank max slots row in the `/pokebank status` table when bank storage is unlimited.

- **Default:** `Unlimited`

##### valueEnabled

The `valueEnabled` message is shown as the value for an enabled option in the `/pokebank status` table.

- **Default:** `§aEnabled`

##### valueDisabled

The `valueDisabled` message is shown as the value for a disabled option in the `/pokebank status` table.

- **Default:** `§cDisabled`